...
def cancel_user_pending_deposit(uid: int) -> int:
    conn = db_connect()
    cur = conn.execute(
        "UPDATE pending_deposits SET status='expired' WHERE user_id=? AND status='pending'",
        (uid,)
    )
    conn.commit()
    conn.close()
...
def generate_unique_deposit_amount(base_amount, network: str) -> str:
    ...
    # Binance Pay: exact amount, no noise
    if net == "BINANCE":
        base_dec = parse_amount_decimal(base_amount)
        if base_dec is None:
            raise ValueError("Invalid deposit amount")
        return str(base_dec)
    
    # TRC20 / BEP20: 4 decimal places, noise only in positions 3-4
    # e.g. user enters 10 → 10.0042, user enters 10.11 → 10.1137
    base_dec = normalize_amount_decimal(base_amount, net)
    if base_dec is None:
        raise ValueError("Invalid deposit amount")
    
    # Quantize base to 2 decimal places (preserve user's cents)
    base_2dp = base_dec.quantize(Decimal("0.01"), rounding=ROUND_DOWN)
    
    for _ in range(200):
        noise = Decimal(random.randint(1, 99)).scaleb(-4)  # 0.0001 to 0.0099
        unique = base_2dp + noise
        unique_str = format(unique, ".4f")
        conn = db_connect()
        ...
        if not existing:
            return unique_str
    # Fallback
    fallback = base_2dp + Decimal(random.randint(1, 99)).scaleb(-4)
    return format(fallback, ".4f")
...
# Legacy RPC fetchers removed in favor of Binance internal Spot API
...
async def verify_pending_deposit_tx(deposit: dict, txid: str) -> dict:
    ...
    if network == "BINANCE":
        ...
        tx_info = await asyncio.to_thread(verify_binance_pay_order, txid_norm)
        if not tx_info:
            spot_amount = await asyncio.to_thread(verify_binance_spot_deposit, txid_norm, "BINANCE")
            if spot_amount:
                tx_info = {"amount": spot_amount}
        
        if not tx_info:
            if txid_norm.isdigit() and len(txid_norm) > 10:
                return {
                    "ok": True,
                    "binance_manual": True,
                    "txid": txid_norm,
                    "amount_decimal": pending_expected_amount_decimal(deposit)
                }
        ...
        # Validate transaction time to prevent double-spending with old transactions
        # Binance returns transactionTime in ms
        tx_time_ms = tx_info.get("transactionTime")
        if tx_time_ms:
            tx_dt = datetime.datetime.utcfromtimestamp(int(tx_time_ms) / 1000)
            expires_dt = parse_db_datetime(deposit.get("expires_at"))
            # Deposit request creation time is expires_at - 30 minutes
            if expires_dt:
                created_dt = expires_dt - datetime.timedelta(minutes=30)
                # If transaction is older than deposit creation minus 5 minutes buffer, reject it
                # Because the user can't claim a new deposit with a very old transaction.
                if tx_dt < created_dt - datetime.timedelta(minutes=15):
                    return {"ok": False, "reason": "already_used"}

        # We also record orderId in database via _confirm_deposit, so normal double-spend also triggers.
        return {
            "ok": True,
            "txid": txid_norm,
    ...
    if network in ("TRC20", "BEP20", "ETH", "TON"):
        from services.binance_pay_api import verify_binance_spot_deposit
        tx_data_str = await asyncio.to_thread(verify_binance_spot_deposit, txid_norm, network)
        if not tx_data_str:
            return {"ok": False, "reason": "not_found"}

        actual_amount = parse_amount_decimal(tx_data_str)
        expected_amount = pending_expected_amount_decimal(deposit)
        ...
        if actual_amount is None or actual_amount != expected_amount:
            return {
                "ok": False,
                "reason": "amount_mismatch",
                "found_amount": decimal_to_display(actual_amount) if actual_amount is not None else str(tx_data_str),
            }
        ...
        return {
            ...
            "amount_text": decimal_to_display(actual_amount),
            "network": network,
        }
