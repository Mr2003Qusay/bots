...
async def verify_pending_deposit_tx(deposit: dict, txid: str) -> dict:
    ...
    if network == "BINANCE":
        ...
        logger.info(f"[BINANCE CLAIM] Verifying order_id={txid_norm} for deposit_id={deposit.get('id')}")
        
        tx_info = await asyncio.to_thread(verify_binance_pay_order, txid_norm)
        logger.info(f"[BINANCE CLAIM] verify_binance_pay_order result: {tx_info}")
        
        if not tx_info:
            spot_amount = await asyncio.to_thread(verify_binance_spot_deposit, txid_norm, "BINANCE")
            logger.info(f"[BINANCE CLAIM] verify_binance_spot_deposit result: {spot_amount}")
            if spot_amount:
                tx_info = {"amount": spot_amount}
        
        if not tx_info:
            logger.warning(f"[BINANCE CLAIM] NOT FOUND for order_id={txid_norm}")
