...
from config import (
    ...
    MY_TRC20_ADDRESS, MY_BEP20_ADDRESS, logger,
)
...
from models.shop import (
    ...
    get_purchase_count, get_available_code_count,
)
...
async def cmd_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("💠 USDT TON", callback_data="dep_ton"),
            InlineKeyboardButton("🔷 USDT ETH", callback_data="dep_eth"),
        ],
    ])
...
async def handle_deposit_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if existing:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Cancel Current Request", callback_data="cancel_deposit")]])
    ...
    from config import MY_BINANCE_ID, MY_ETH_ADDRESS, MY_TON_ADDRESS
    if network == "TRC20":
        wallet = MY_TRC20_ADDRESS
    elif network == "BEP20":
        wallet = MY_BEP20_ADDRESS
    elif network == "ETH":
        wallet = MY_ETH_ADDRESS
    elif network == "TON":
        wallet = MY_TON_ADDRESS
    else:
        wallet = MY_BINANCE_ID
        
    expires_at = (datetime.datetime.utcnow() + datetime.timedelta(minutes=30)).isoformat()

    # Generate unique deposit code for Binance Pay
    deposit_code = ""
    if network == "BINANCE":
        import secrets as _sec
        deposit_code = f"TOOLS-{_sec.token_hex(3).upper()}"
        # Make sure code is unique
        conn_check = db_connect()
        while conn_check.execute(
            "SELECT 1 FROM pending_deposits WHERE deposit_code=? AND status='pending'",
            (deposit_code,)
        ).fetchone():
            deposit_code = f"TOOLS-{_sec.token_hex(3).upper()}"
        conn_check.close()
    ...
    conn = db_connect()
    conn.execute(
        "INSERT INTO pending_deposits (user_id, network, expected_amount, expected_amount_str, base_amount, wallet_address, expires_at, deposit_code) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (uid, network, float(parse_amount_decimal(unique_amount_str) or 0),
         unique_amount_str, float(base_amount), wallet, expires_at, deposit_code)
    )
    conn.commit()
    conn.close()

    if network == "TRC20":
        key = "deposit_pending_trc20"
    elif network == "BEP20":
        key = "deposit_pending_bep20"
    else:
        key = "deposit_pending_binance"
        
    await update.message.reply_text(
        t(uid, key, amount=unique_amount_str, wallet=wallet, code=deposit_code),
        parse_mode="HTML"
    )
...
async def process_deposit_claim(update: Update, context: ContextTypes.DEFAULT_TYPE, raw_txid: str):
    ...
    raw = (raw_txid or "").strip()

    # ── Handle TOOLS-XXXXXX code claims ──────────────────────────────
    if raw.upper().startswith("TOOLS-"):
        ...
        stored_code = (pending.get("deposit_code") or "").upper()
        if stored_code != code:
            return await message.reply_text(
                "❌ <b>Invalid code!</b>\nThis code does not match your pending deposit.",
                parse_mode="HTML"
            )
        ...
        # Code matches! Notify admin and tell user to wait for auto-detection
        from handlers.common import send_log_via_second_bot
        amount_str = pending_expected_amount_str(pending)
        admin_msg = (
            f"💰 <b>Binance Pay Deposit — Code Confirmed</b>\n\n"
            f"👤 User: <code>{uid}</code>\n"
            f"💵 Expected: <b>{amount_str} USDT</b>\n"
            f"📝 Code: <code>{code}</code>\n\n"
            f"Waiting for email auto-detection to confirm amount.\n"
            f"If stuck, use: <code>/addbalance {uid} {amount_str}</code>"
        )
        await send_log_via_second_bot(admin_msg)
        
        return await message.reply_text(
            "✅ <b>Code Verified!</b>\n\n"
            "🔍 The system is now scanning for your payment.\n"
            "Your balance will be updated automatically within 1-2 minutes.\n\n"
            "⏳ Please wait...",
            parse_mode="HTML"
        )

    # ── Standard TXID claim flow ─────────────────────────────────────
    pending = get_user_pending_deposit(uid)
    if not pending:
        return await message.reply_text(t(uid, "deposit_no_pending"), parse_mode="HTML")
    ...
    txid_norm = canonicalize_txid(raw, pending.get("network", "TRC20"))
    if not txid_norm:
    ...
    if result.get("binance_manual"):
        from handlers.common import send_log_via_second_bot
        amount_str = pending_expected_amount_str(pending)
        admin_msg = (
            f"💰 <b>Binance Pay Deposit Review</b>\n\n"
            f"👤 User: <code>{uid}</code>\n"
            f"💵 Expected: <b>{amount_str} USDT</b>\n"
            f"🧾 <b>Binance ID / Order:</b> <code>{result['txid']}</code>\n\n"
            f"Please verify manually and use `/addbalance {uid} {amount_str}` if valid."
        )
        await send_log_via_second_bot(admin_msg)
        try:
            await status_msg.edit_text(
                "⏳ <b>Payment ID Received!</b>\n\n"
                "Binance Pay deposits are verified manually by our team. "
...
async def finalize_purchase(update, context, user_id, shop_id, product, qty, cost):
    """Hybrid delivery: deliver available codes first, then manual for the rest."""
    ...
    # Try to claim codes first (regardless of delivery_type)
    available = get_available_code_count(shop_id, pid)
    if available > 0:
        codes = claim_product_codes(shop_id, pid, user_id, qty)
        manual_qty = qty - len(codes)

    # If no codes were used and mode is manual, just reduce stock
    if not codes and mode != "codes":
        reduce_stock(shop_id, pid, qty)

    delivery_data = "\n".join(codes) if codes else ""
    record_purchase(shop_id, user_id, pid, float(cost), int(qty),
                    "Hybrid" if codes and manual_qty > 0 else ("Instant" if codes else "Manual"),
                    delivery_data)
    ...
    if manual_qty > 0:
        ...
        if codes:
            manual_text = (
                f"📦 <b>Partial Manual Delivery</b>\n\n"
                f"✅ {len(codes)} code(s) delivered instantly.\n"
                f"⏳ <b>{manual_qty}</b> remaining — codes are out of stock.\n"
                f"Please contact {SUPPORT_USER} to receive the rest."
            )
        else:
            manual_text = (
                f"📦 <b>Manual Delivery Required</b>\n\n"
                f"Codes are currently out of stock.\n"
                f"Please contact {SUPPORT_USER} to receive your order."
            )
        await update.effective_message.reply_text(manual_text, parse_mode="HTML", reply_markup=contact_kb)

    # ── File delivery ──
    if mode == "file" and product.get("file_id") and not codes:
        try:
    ...
    # ── Admin log ──
    log_label = "Sale"
    if codes and manual_qty > 0:
        log_label = f"Sale (Hybrid: {len(codes)} codes + {manual_qty} manual)"
    elif not codes:
        log_label = "Sale (Manual)"
