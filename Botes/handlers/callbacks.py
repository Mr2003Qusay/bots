...
from config import (
    OWNER_ID, MAIN_BOT_TOKEN, MAINTENANCE_MODE,
    SUPPORT_USER, logger,
)
...
async def callback_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("dep_trc20", "dep_bep20", "dep_eth", "dep_ton", "dep_binance"):
        if mode != "main_user":
            return await query.answer(t(uid, "activate_not_available"), show_alert=True)
    ...
    if data == "cancel_deposit":
        if mode != "main_user":
            return await query.answer()
        from services.blockchain import cancel_user_pending_deposit
        cancel_user_pending_deposit(uid)
        await query.answer("Deposit Request Cancelled.")
        try:
            await query.message.edit_text(
                "🚫 <b>Your pending deposit request was successfully cancelled.</b>\n"
                "You can now create a new deposit request.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💰 Start New Deposit", callback_data="user_deposit")]])
            )
        except Exception:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("state") == "WAIT_QTY":
        ...
        if get_shop_balance(uid, shop_id) < total_cost:
            ...
            nav_kb = InlineKeyboardMarkup([[InlineKeyboardButton("💰 Deposit Now", callback_data="user_deposit")]])
            return await update.message.reply_text(
                t(uid, "prod_no_bal", name=product["name"]), 
        ...
        return await update.message.reply_text(
            t(uid, "prod_buy_confirm", name=product["name"], qty=qty, total=float(total_cost), pid=pid),
            parse_mode="HTML"
        )
