...
def update_product(shop_id: int, product_id: int, **kwargs):
    """Update product fields. Allowed keys: name, price, description, category, image_id, hidden."""
    allowed = {"name", "price", "description", "category", "image_id", "hidden"}
...
def get_product_sales(shop_id: int, product_id: int, limit: int = 10):
    ...
    conn = db_connect()
    rows = conn.execute("""
        SELECT p.user_id, p.qty, p.price, p.created_at, u.username
        FROM purchases p
